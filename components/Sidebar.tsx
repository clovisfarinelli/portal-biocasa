    ...
    <aside className={`flex flex-col bg-escuro-700 border-r border-escuro-500 transition-all duration-300 ${recolhido ? 'w-16' : 'w-64'}`}>
      ...
      <nav className="flex-1 px-3 py-4 space-y-1.5 overflow-y-auto">
        ...
        {temImoveis && (
          <div>
            ...
            {moduloAberto === 'IMOB' && !recolhido && (
              <div className="mt-1 ml-3 pl-3 border-l-2 border-dourado-400/30 space-y-0.5">
                <Link href="/imoveis" className={btnSub(pathname.startsWith('/imoveis') && !pathname.startsWith('/imoveis/relatorios'))}>
                  <svg className="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                {isAdmin && (
                  <Link href="/imoveis/relatorios" className={btnSub(pathname.startsWith('/imoveis/relatorios'))}>
                    <svg className="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    </svg>
                    Relatórios
        ...
        {perfil === 'MASTER' && (
          <Link
            href="/consolidado"
            className={btnModulo(pathname.startsWith('/consolidado'))}
            title="Dashboard"
          >
            <svg className="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {!recolhido && (
              <span className="flex-1 text-left uppercase tracking-widest text-xs">Dashboard</span>
            )}
          </Link>
        )}
